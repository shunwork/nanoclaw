...
│  │  (grammy events) │    │  (checks tasks)  │    │  (file-based) │  │
│  └────────┬─────────┘    └────────┬─────────┘    └───────────────┘  │
│           │                       │                                  │
│           └───────────┬───────────┘                                  │
│                       │ spawns container                             │
...
|-----------|------------|---------|
| Telegram Bot | grammy | Connect to Telegram Bot API, send/receive messages |
...
│
├── src/
│   ├── index.ts                   # Main application (Telegram bot + routing)
│   ├── config.ts                  # Configuration constants
│   ├── types.ts                   # TypeScript interfaces
│   ├── utils.ts                   # Generic utility functions
│   ├── db.ts                      # Database initialization and queries
│   ├── telegram-auth.ts           # Standalone Telegram bot authentication
...
1. User sends Telegram message
   │
   ▼
2. grammy receives message via Telegram Bot API (long polling)
   │
   ▼
3. Message stored in SQLite (store/messages.db) and immediately enqueued
   │
   ▼
4. Router checks:
...
8. Claude processes message:
   ...
   │
   ▼
8. Router prefixes response with assistant name and sends via Telegram
   │
   ▼
9. Router updates last agent timestamp and saves session ID
```
...
4. Connects to Telegram Bot API
5. Registers message event handlers
...
| Telegram Bot Token | `.env` file | Set via `TELEGRAM_BOT_TOKEN`, never mounted into containers |
...
| "Bot token invalid" | Telegram token expired or wrong | Update TELEGRAM_BOT_TOKEN in .env and restart |
